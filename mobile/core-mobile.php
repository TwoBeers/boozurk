...
if ( !function_exists( 'boozurk_mobile' ) ) {
	function boozurk_mobile () {
		...
		if ( $boozurk_is_mobile_browser ) {
			...
			// Custom filters
			add_filter( 'user_contactmethods','boozurk_mobile_new_contactmethods',10,1 );
			add_filter( 'widget_tag_cloud_args', 'boozurk_mobile_tag_cloud_filter', 90 );
			add_filter( 'widget_categories_args', 'boozurk_mobile_widget_categories_filter', 90 );
			add_filter( 'widget_archives_args', 'boozurk_mobile_widget_archives_filter', 90 );
			add_filter( 'widget_pages_args', 'boozurk_mobile_widget_pages_filter', 90 );
			add_filter( 'boozurk_widget_pop_categories_args', 'boozurk_mobile_widget_pop_categories_filter', 90 );
...
if ( !function_exists( 'boozurk_mobile_widget_area_init' ) ) {
	function boozurk_mobile_widget_area_init() {
		// Area 0, in the tbm sidebar.
		register_sidebar( array(
			'name' => __( 'Mobile Widget Area', 'boozurk' ),
			'id' => 'tbm-widget-area',
			'description' => '',
			'before_widget' => '<div id="%1$s" class="widget %2$s"><div class="widget-body">',
			'after_widget' => '</div></div>',
			'before_title' => '</div>' . boozurk_mobile_seztitle( 'before' ),
			'after_title' => boozurk_mobile_seztitle( 'after' ) . '<div class="widget-body">',
		) );
...
if ( !function_exists( 'boozurk_mobile_seztitle' ) ) {
	function boozurk_mobile_seztitle( $a ){
		if ( $a == 'before' ) 
...
if ( !function_exists( 'boozurk_mobile_post_details' ) ) {
	function boozurk_mobile_post_details( $auth, $date, $tags, $cats, $hiera = false, $av_size = 48, $featured = false ) {
		global $post;
		?>
...
if ( !function_exists( 'boozurk_mobile_setup' ) ) {
	function boozurk_mobile_setup() {
...
function boozurk_mobile_tag_cloud_filter($args = array()) {
   $args['smallest'] = 1;
   $args['largest'] = 1;
   $args['unit'] = 'em';
   return $args;
...
function boozurk_mobile_widget_categories_filter($args = array()) {
   $args['hierarchical'] = 0;
   $args['show_count'] = 0;
   return $args;
}

function boozurk_mobile_widget_archives_filter($args = array()) {
   $args['show_post_count'] = 0;
   return $args;
}

function boozurk_mobile_widget_pages_filter($args = array()) {
   $args['depth'] = 1;
   return $args;
}

function boozurk_mobile_widget_pop_categories_filter($args = array()) {
   $args['show_count'] = 0;
   return $args;
}
