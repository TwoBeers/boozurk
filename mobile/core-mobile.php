...
class Boozurk_Mobile {
	...
	function indexes_navigation(){
		...
		echo apply_filters( 'boozurk_mobile_filter_seztitle', sprintf( __( 'page %1$s of %2$s', 'boozurk' ) , $paged, $wp_query->max_num_pages ) );
	...
	function page_hierarchy(){
		...
		if ( !empty($sub_pages) ) {
			...
			?>
				<?php echo apply_filters( 'boozurk_mobile_filter_seztitle', __( 'Child pages', 'boozurk' ) ); ?>
				<ul class="tbm-group">
					<?php 
		...
		if ( $parent_page ) {
			?>
				<?php echo apply_filters( 'boozurk_mobile_filter_seztitle', __( 'Parent page', 'boozurk' ) ); ?>
				<ul class="tbm-group">
	...
	function search_reminder() {
		...
		if ( is_archive() ) {
			...
			if ( is_category() || is_tag() || is_tax() ) {
				if ( is_category() )	$type = __( 'Category', 'boozurk' );
				elseif ( is_tag() )		$type = __( 'Tag', 'boozurk' );
				elseif ( is_tax() )		$type = __( 'Taxonomy', 'boozurk' );
				$title = $term->name;
			} elseif ( is_date() ) {
				$type = __( 'Date', 'boozurk' );
				if ( is_day() ) {
					$title = get_the_date();
				} else if ( is_month() ) {
			...
			$text = sprintf( __( 'Search results for &#8220;%s&#8221;', 'boozurk' ), '<span class="search-term">' . esc_html( get_search_query() ) . '</span>' );

		} elseif ( is_404() ) {

			$text = __( 'Error 404', 'boozurk' );

		}

		echo apply_filters( 'boozurk_mobile_filter_seztitle', $text );
	...
	function comment_form_defaults( $defaults ) {
		...
		$defaults['title_reply']		= apply_filters( 'boozurk_mobile_filter_seztitle', __( 'Leave a comment', 'boozurk' ) );
		$defaults['title_reply_to']		= apply_filters( 'boozurk_mobile_filter_seztitle', __( 'Leave a Reply to %s', 'boozurk' ) );
	...
	function body_classes( $classes ) {
		...
		$classes[] = $this->get_option( 'boozurk_mobile_css_color' );
