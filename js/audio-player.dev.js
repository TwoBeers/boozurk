var boozurkAudioPlayer;

(function($) {

	boozurkAudioPlayer = function () {
		var instances = [];
		var activePlayerID;
		var playerURL = "";
		var defaultOptions = {};
		...
		return {
			...
			init : function() {
				...
				boozurkAudioPlayer.setup(boozurkAudioPlayer_l10n.player_path, {
					width: 300,
					loop: "yes",
					...
					if ( the_source.size() !== 0 ) {
						...
						case '.ogg':
							if ( !document.createElement("audio").canPlayType ) {
								$this.parent().html('<span class="bz-player-notice">' + boozurkAudioPlayer_l10n.unknown_media + '</span>');
							}
							break;
						case '.mp3':
							if ( !document.createElement("audio").canPlayType || (document.createElement("audio").canPlayType && !document.createElement("audio").canPlayType('audio/mpeg')) ) {
								boozurkAudioPlayer.embed(this.id, {  
									soundFile: the_href
								});  
							}
							break;
	...
	$(document).ready(function($){ boozurkAudioPlayer.init(); });
